...
void print_to_file(string, double*, int, int);
...
int main(int argc, const char * argv[]) {
    ...
    int L = 1;
    int x_steps = 10; //dx = 0.1
    int time_steps = 100;
    double dx = (double)L/x_steps; double dt = 0.4*dx*dx;
    ...
    u[x_steps-1] = u_new[x_steps-1] = 1;
    
    
    for(int j=1; j<time_steps; j++){
        if (method == 1){
            Forward_Euler(x_steps, alpha, u, u_new);
            outfile_name = "Forward_Euler";
        }
        else if (method == 2){
            Backward_Euler(x_steps, alpha, u, u_new);
            outfile_name = "Backward_Euler";
        }
        else if (method == 3){
            Crank_Nicolson(x_steps, alpha, u);
            outfile_name = "Crank-Nicolson";
        }
        
        // Print to file
        if (j==0.5*time_steps){
            print_to_file(outfile_name, u, x_steps, j);
        }
        else if (j==0.8*time_steps){
            print_to_file(outfile_name, u, x_steps, j);
        }
    }
...
void print_to_file(string outfile_name, double *u, int n, int j){
    string outfile_name1 = outfile_name;
    outfile_name1.append("_t="); outfile_name1.append(to_string(j));
    outfile_name1.append(".txt");
    ofile.open(outfile_name1);
...
void Crank_Nicolson(int Number_of_XSteps, double alpha, double *u){
    ...
    utemp[0] = 0; utemp[Number_of_XSteps-1] = 1;
